...
const DEFAULT_TITLES = {
  plan: "Antigravity Plan",
  review: "Antigravity Review",
  "adversarial-review": "Antigravity Adversarial Review",
  rescue: "Antigravity Rescue"
};
...
function extractAntigravityEnvelope(input, rawOutput) {
...
function pickAntigravityText(input, envelope, rawOutput) {
  if (isPlainObject(input)) {
    if (input.text !== undefined) {
      return input.text;
    }
...
function buildError(input, status, stderr) {
  ...
  if (input.timedOut) {
    return "Antigravity command timed out";
  }
  if (status === "failed") {
    return compactString(stderr) || "Antigravity command failed";
  }
  return null;
}
...
function renderPlan(normalized) {
  const lines = [
    ...
    normalized.sessionId ? `Antigravity session: \`${normalized.sessionId}\`` : "",
...
export function normalizeCompanionResult(input = {}, options = {}) {
  ...
  const { envelope, parseError: envelopeParseError } = extractAntigravityEnvelope(source, rawOutput);
  const agyText = pickAntigravityText(source, envelope, rawOutput);
  const { structured, parseError: structuredParseError, text } = extractStructured(agyText);
  ...
  const normalized = {
    ...
    sessionId: firstNonEmpty(source.sessionId, source.agySessionId, envelope?.session_id, envelope?.sessionId, structured.sessionId) || null,
    error,
    metadata,
...
export function renderSetupResult(status, options = {}) {
  ...
  const lines = [
    `Antigravity Bridge setup: ${ready ? "ready" : "not ready"}`,
    payload.antigravity?.available === false ? "Antigravity: not found" : "",
    payload.antigravity?.version?.stdout ? `Antigravity: ${payload.antigravity.version.stdout.trim()}` : ""
  ];
...
export function renderStatus(snapshot = {}, options = {}) {
  ...
  if (rows.length === 1) {
    return "No Antigravity companion jobs recorded.";
  }
  return rows
    .map((row, index) => {

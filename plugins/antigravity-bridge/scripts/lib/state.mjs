...
export function resolveStateRoot(env = process.env) {
  return env.ANTIGRAVITY_COMPANION_STATE_DIR || env.CODEX_PLUGIN_DATA || env.ANTIGRAVITY_PLUGIN_DATA || path.join(os.tmpdir(), "antigravity-companion");
}
...
function minimalStorageFallbackResult(payload, policy, omittedBytes) {
  ...
  const result = {
    ...
    actions: [],
    touchedFiles: [],
    sessionId: source.sessionId ?? source.agySessionId ?? null,
